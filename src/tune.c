...
/* ══════════════════════════════════════════════════════════════════════
 * save_weights helpers
 * ══════════════════════════════════════════════════════════════════════ */

/* Emit one 8×8 PST exactly as it appears in eval.c:
 *   static const int PST_PAWN_MG[64] = {
 *           0,     0, ...
 *   };
 * Column width matches eval.c (6 chars per value). */
static int emit_pst(FILE *f, const char *name, const int sq[64])
{
    ...
    for (int r = 0; r < 8; r++) {
        ...
        for (int c = 0; c < 8; c++) {
            int last = (r == 7 && c == 7);
            if (fprintf(f, "%6d%s", sq[r*8+c], last ? "\n" : ",") < 0) return -1;
        }
    }
    return fputs("};\n", f) == EOF ? -1 : 0;
}

/* Emit a short 1-D array on a single line, optionally with column alignment:
 *   static const int MOB_KNIGHT_MG[] = {-62,-53,...};
 * width=0 → compact; width>0 → each value right-justified to that width. */
static int emit_arr1(FILE *f, const char *name, const int *v, int n, int width)
{
    if (fprintf(f, "static const int %s[] = {", name) < 0) return -1;
    for (int i = 0; i < n; i++) {
        if (width > 0) {
            if (fprintf(f, "%*d%s", width, v[i], i<n-1?",":"") < 0) return -1;
        } else {
...
/* Emit a [6][6] quad table in the same block-row style as eval.c. */
static int emit_quad(FILE *f, const char *name, const int v[36])
{
    if (fprintf(f, "static const int %s[6][6] = {\n", name) < 0) return -1;
    for (int r = 0; r < 6; r++) {
        if (fputs("    {", f) == EOF) return -1;
        for (int c = 0; c < 6; c++) {
            int last = (r == 5 && c == 5);
            if (fprintf(f, "%5d%s", v[r*6+c], (c<5)?",":(last?"}":",}")) < 0) return -1;
        }
        /* trailing comma on every row except last */
        if (fprintf(f, "%s\n", r < 5 ? "," : "") < 0) return -1;
    }
    return fputs("};\n", f) == EOF ? -1 : 0;
}
...
/* ══════════════════════════════════════════════════════════════════════
 * save_weights — emits a drop-in replacement for the constant block at
 * the top of eval.c.  The output can be copied verbatim; each variable
 * name and array dimension matches what eval.c's code already reads.
 *
 * Hardcoded magic numbers that live inside function bodies (TrappedRook,
 * WeakQueen, KingProtector, MinorBehindPawn, fork/skewer literals in
 * eval_threats) are listed in a NOTE block at the bottom of the file
 * for manual review.
 *
 * Returns 0 on success, -1 on I/O error.
 * ══════════════════════════════════════════════════════════════════════ */
...
#define W(expr) do { if ((expr) < 0) goto io_err; } while(0)
#define WS(s)   do { if (fputs((s), f) == EOF) goto io_err; } while(0)

    /* ── Header ──────────────────────────────────────────────────────── */
    W(fprintf(f,
        "/*\n"
        " * tuned_weights_checkpoint.c\n"
        " * Auto-generated by save_weights() — paste these declarations\n"
        " * over the matching blocks at the top of eval.c.\n"
        " */\n\n"));

    /* ── Material ────────────────────────────────────────────────────── */
    WS("/* ── Material ─────────────────────────────────────────────── */\n");
    W(emit_arr1(f, "MATERIAL_MG[6]", w->material_mg, 6, 4));
    W(emit_arr1(f, "MATERIAL_EG[6]", w->material_eg, 6, 4));
    WS("\n");

    /* ── Piece-square tables ─────────────────────────────────────────── */
    static const char *PIECE[6] = {
        "PAWN", "KNIGHT", "BISHOP", "ROOK", "QUEEN", "KING"
    };
    static const char *PH[2] = { "MG", "EG" };
    const int (*pst_tbl[2])[64] = {
        (const int (*)[64])w->pst_mg,
        (const int (*)[64])w->pst_eg
    };

    WS("/* ── Piece-square tables ───────────────────────────────────── */\n");
    for (int pt = 0; pt < 6; pt++) {
        for (int ph = 0; ph < 2; ph++) {
            char name[32];
            snprintf(name, sizeof name, "PST_%s_%s", PIECE[pt], PH[ph]);
            W(emit_pst(f, name, pst_tbl[ph][pt]));
        }
        WS("\n");
    }
    ...
    WS("/* ── Pawn structure ────────────────────────────────────────── */\n");
    W(fprintf(f, "static const int DOUBLED_PAWN_PENALTY_MG  = %d;\n", w->doubled_pawn_mg));
    W(fprintf(f, "static const int DOUBLED_PAWN_PENALTY_EG  = %d;\n", w->doubled_pawn_eg));
    W(fprintf(f, "static const int ISOLATED_PAWN_PENALTY_MG = %d;\n", w->isolated_pawn_mg));
    W(fprintf(f, "static const int ISOLATED_PAWN_PENALTY_EG = %d;\n", w->isolated_pawn_eg));
    W(fprintf(f, "static const int BACKWARD_PAWN_PENALTY_MG = %d;\n", w->backward_pawn_mg));
    W(fprintf(f, "static const int BACKWARD_PAWN_PENALTY_EG = %d;\n", w->backward_pawn_eg));
    W(emit_arr1(f, "PASSED_PAWN_BONUS_MG[8]", w->passed_pawn_mg, 8, 4));
    W(emit_arr1(f, "PASSED_PAWN_BONUS_EG[8]", w->passed_pawn_eg, 8, 4));
    WS("\n");

    /* ── Mobility tables ─────────────────────────────────────────────── */
    WS("/* ── Mobility (SF 11 non-linear tables) ───────────────────── */\n");
    W(emit_arr1(f, "MOB_KNIGHT_MG[]", w->mob_knight_mg,  9, 4));
    W(emit_arr1(f, "MOB_KNIGHT_EG[]", w->mob_knight_eg,  9, 4));
    WS("\n");
    W(emit_arr1(f, "MOB_BISHOP_MG[]", w->mob_bishop_mg, 14, 4));
    W(emit_arr1(f, "MOB_BISHOP_EG[]", w->mob_bishop_eg, 14, 4));
    WS("\n");
    W(emit_arr1(f, "MOB_ROOK_MG[]",   w->mob_rook_mg,   15, 4));
    W(emit_arr1(f, "MOB_ROOK_EG[]",   w->mob_rook_eg,   15, 4));
    WS("\n");
    /* Queen tables are wider — wrap at 14 values to match eval.c style */
    W(fprintf(f, "static const int MOB_QUEEN_MG[] = {\n    "));
    for (int i = 0; i < 28; i++)
        W(fprintf(f, "%3d%s%s", w->mob_queen_mg[i],
                  i<27?",":" ", (i==13)?"\n    ":""));
    WS("};\n");
    W(fprintf(f, "static const int MOB_QUEEN_EG[] = {\n    "));
    for (int i = 0; i < 28; i++)
        W(fprintf(f, "%3d%s%s", w->mob_queen_eg[i],
                  i<27?",":" ", (i==13)?"\n    ":""));
    WS("};\n\n");

    /* ── Rook bonuses ────────────────────────────────────────────────── */
    WS("/* ── Rook bonuses ──────────────────────────────────────────── */\n");
    W(fprintf(f, "static const int ROOK_OPEN_FILE_MG  = %d;\n", w->rook_open_mg));
    W(fprintf(f, "static const int ROOK_OPEN_FILE_EG  = %d;\n", w->rook_open_eg));
    W(fprintf(f, "static const int ROOK_SEMIOPEN_MG   = %d;\n", w->rook_semi_mg));
    W(fprintf(f, "static const int ROOK_SEMIOPEN_EG   = %d;\n", w->rook_semi_eg));
    W(fprintf(f, "static const int ROOK_ON_SEVENTH_MG = %d;\n", w->rook_seventh_mg));
    W(fprintf(f, "static const int ROOK_ON_SEVENTH_EG = %d;\n", w->rook_seventh_eg));
    WS("\n");

    /* ── Bishop pair ─────────────────────────────────────────────────── */
    WS("/* ── Bishop pair ───────────────────────────────────────────── */\n");
    W(fprintf(f, "static const int BISHOP_PAIR_MG = %d;\n", w->bishop_pair_mg));
    W(fprintf(f, "static const int BISHOP_PAIR_EG = %d;\n", w->bishop_pair_eg));
    WS("\n");

    /* ── Outposts ────────────────────────────────────────────────────── */
    WS("/* ── Outposts ──────────────────────────────────────────────── */\n");
    W(fprintf(f, "static const int OUTPOST_KNIGHT_MG = %d;\n", w->outpost_knight_mg));
    W(fprintf(f, "static const int OUTPOST_KNIGHT_EG = %d;\n", w->outpost_knight_eg));
    W(fprintf(f, "static const int OUTPOST_BISHOP_MG = %d;\n", w->outpost_bishop_mg));
    W(fprintf(f, "static const int OUTPOST_BISHOP_EG = %d;\n", w->outpost_bishop_eg));
    WS("\n");

    /* ── King safety ─────────────────────────────────────────────────── */
    WS("/* ── King safety ───────────────────────────────────────────── */\n");
    W(fprintf(f, "static const int KING_SHIELD_BONUS        = %d;\n", w->king_shield));
    W(fprintf(f, "static const int KING_OPEN_FILE_PENALTY   = %d;\n", w->king_open_file));
    W(emit_arr1(f, "KING_ATTACKER_WEIGHT[6]", w->king_attacker_weight, 6, 4));
    W(fprintf(f, "static const int KING_EG_DISTANCE_PENALTY = %d;\n",
              w->king_eg_distance_penalty));
    WS("\n");

    /* ── Tempo ───────────────────────────────────────────────────────── */
    WS("/* ── Tempo ─────────────────────────────────────────────────── */\n");
    W(fprintf(f, "#define TEMPO_BONUS_MG %d\n", w->tempo_mg));
    W(fprintf(f, "#define TEMPO_BONUS_EG %d\n", w->tempo_eg));
    WS("\n");

    /* ── Hanging-piece penalties ─────────────────────────────────────── */
    WS("/* ── Hanging-piece penalties ───────────────────────────────── */\n");
    W(emit_arr1(f, "HANG_PENALTY_MG[6]", w->hang_penalty_mg, 6, 5));
    W(emit_arr1(f, "HANG_PENALTY_EG[6]", w->hang_penalty_eg, 6, 5));
    WS("\n");

    /* ── Material imbalance (6×6 quad tables) ────────────────────────── */
    WS("/* ── Material imbalance (SF 11 quadratic) ──────────────────── */\n");
    W(emit_quad(f, "QuadOurs",   w->quad_ours));
    W(emit_quad(f, "QuadTheirs", w->quad_theirs));
    WS("\n");

    /* ── NOTE: values that live inside function bodies ───────────────── *
     * These are not static constants in eval.c; they are hardcoded       *
     * literals inside the relevant functions.  The tuner tracks them via *
     * EvalWeights but cannot paste them automatically.  Update manually. */
    WS(
        "/*\n"
        " * NOTE — update these literals manually in eval.c:\n"
        " *\n"
        " * eval_rooks()  (TrappedRook penalty, ~line 524):\n"
        " *   *mg -= 52;  →  *mg -= "
    );
    W(fprintf(f, "%d;\n", w->trapped_rook_mg));
    WS(
    ...
    WS(
        " *\n"
        " * eval_queen_weak()  (WeakQueen penalty, ~line 911):\n"
        " *   *mg -= 49;  →  *mg -= "
    );
    W(fprintf(f, "%d;\n", w->weak_queen_mg));
    WS("  *   *eg -= 15;  →  *eg -= ");
    ...
    WS(
        " *\n"
        " * eval_minor_behind_pawn()  (~line 942):\n"
        " *   *mg += 18 * count;  →  *mg += "
    );
    W(fprintf(f, "%d * count;\n", w->minor_behind_pawn_mg));
    WS("  *   *eg +=  3 * count;  →  *eg += ");
    W(fprintf(f, "%d * count;\n", w->minor_behind_pawn_eg));
    WS(
        " *\n"
        " * eval_threats()  fork/skewer literals  (~line 858):\n"
        " *   *mg += 45 + 20*(hits-2);  →  *mg += "
    );
    W(fprintf(f, "%d + %d*(hits-2);\n", w->fork_base_mg, w->fork_extra_mg));
    WS("  *   *eg += 35 + 15*(hits-2);  →  *eg += ");
    W(fprintf(f, "%d + %d*(hits-2);\n", w->fork_base_eg, w->fork_extra_eg));
    WS(
        " *   rook/queen skewer  *mg += 20;  →  *mg += "
    );
    W(fprintf(f, "%d;\n", w->skewer_rq_mg));
    WS("  *   rook/queen skewer  *eg += 15;  →  *eg += ");
    W(fprintf(f, "%d;\n", w->skewer_rq_eg));
    WS("  *   bishop/queen skewer  *mg += 15;  →  *mg += ");
    W(fprintf(f, "%d;\n", w->skewer_bq_mg));
    WS("  *   bishop/queen skewer  *eg += 10;  →  *eg += ");
    W(fprintf(f, "%d;\n", w->skewer_bq_eg));
    WS(" */\n");
...
#undef WS
    ...
    printf("[save_weights] Wrote tuned weights to '%s'\n", path);
    return 0;
